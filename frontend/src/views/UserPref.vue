...
async function submitPreferences() {
  ...
  try {
    // Use the Kong API Gateway URL (replace with your actual Kong URL if different)
    const apiUrl = `http://localhost:8000/api/user/${user}`;
    
    // Create payload with taste_preferences as expected by the user service
    const payload = {
      taste_preferences: {
        travel_style: form.travel_style,
        tourist_sites: form.tourist_sites,
        diet: form.diet,
        allergy_detail: form.allergy_detail,
